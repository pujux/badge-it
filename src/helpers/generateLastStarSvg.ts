...
export default function generateLastStarSvg(response: GitHubStarredRepo[], gap: number, perRow: number): string {
  ...
  const repos = response
    .map((repo, i) => {
      const x = (i % perRow) * (width + gap) + wrapperPadding;
      const y = Math.floor(i / perRow) * (height + gap) + wrapperPadding;
  ...
  return `
  	<svg width="${((response.length - 1) % perRow) * (width + gap) + width + wrapperPadding * 2}" height="${
    Math.floor((response.length - 1) / perRow) * (height + gap) + height + wrapperPadding * 2
  }" role="img"
